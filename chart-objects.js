...
class Rect {
	constructor(pointA, pointB){
		...
		this.isPointInside = ({x:px, y: py}, padding=undefined) => {
...
class LinearChart extends ChartObject {
	constructor(name="", yIsInputAxis=false, xOrYRefObj=null, zRefObj=null, ...lines){
		...
		this.calc = (chartObjectArray, alertsArray, noticesArray) => {
		...
		this.figureChart = (xOrYIn, zIn) => {
			...
			const aboveSegment = aboveLine.findSegment(xOrYIn);
			let aboveCrossing=aboveSegment ? aboveSegment.figureYCrossing(xOrYIn) : null;

			if (linesAreSame) return aboveCrossing;

			const belowSegment = belowLine.findSegment(xOrYIn);
			let belowCrossing=belowSegment ? belowSegment.figureYCrossing(xOrYIn) : null;
...
class ClampChart extends ChartObject {
	...
	super(name, "clamp", null);
		...
		this.figureClamp = (xOrYIn, valueToClamp) => {
			if (isAnyNil(xOrYIn, valueToClamp)) return null;
			
			if (this.minLine && this.minLine.points.length>0) {
				const minSeg = this.minLine.findSegment(xOrYIn);
				const minValue = minSeg ? minSeg.figureYCrossing(xOrYIn) : null;
				if (!minValue) return null;
		
				if (valueToClamp < minValue) valueToClamp = minValue;
			}

			if (this.maxLine && this.maxLine.points.length>0) {
				const maxSeg = this.maxLine.findSegment(xOrYIn);
				const maxValue = maxSeg ? maxSeg.figureYCrossing(xOrYIn) : null;
...
class TrendChart extends ChartObject{
	constructor(name="", yIsInputAxis=false, xOrYRefObj=null, entryPointRefObj=null, exitPointRefObj=null, ...lines){
		...
		this.figureTrend = (xOrYIn, entryPoint, exitPoint) => {
			...
			for (let currentLine of this.lines){
				let currentSegment = currentLine.findSegment(entryPoint);
				let yCrossing = currentSegment ? currentSegment.figureYCrossing(entryPoint) : null;
			...
			const leftEndSeg = leftLineFind.findSegment(exitPoint)
			const leftEndCross = leftEndSeg ? leftEndSeg.figureYCrossing(exitPoint) : null;
			if (leftLineFind===rightLineFind || isAboutEquals(rightEntryCross, leftEntryCross)){
				return leftEndCross;
			}

			const rightEndSeg = rightLineFind.findSegment(exitPoint);
			const rightEndCross = rightEndSeg ? rightEndSeg.figureYCrossing(exitPoint) : null;
...
class ChartTable extends ChartObject {
	constructor(name="", inputRefObj=null, ...table){
		...
		this.figureTable = (inValue) => {
			...
			if (above===null || above.refObj===null || above.refObj.value===null) return null;
			if (below===null || below.refObj===null || below.refObj.value===null) return null;
